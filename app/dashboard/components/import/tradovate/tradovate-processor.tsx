...
export default function TradovateProcessor({ headers, csvData, processedTrades = [], setProcessedTrades, accountNumbers }: PlatformProcessorProps) {
    ...
    const applyCommissions = () => {
        ...
        toast.success("Commissions Applied", {
            description: "Commissions have been updated for the processed trades."
        });
    };
                ...
                <div className="space-y-4 p-6">
                    {showCommissionPrompt && (
                        <div className="flex-none bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded-r" role="alert">
                            <p className="font-bold">Enter Missing Commissions</p>
                            <p>We couldn't find commissions for some instruments. Please enter the commission per round-trip contract below.</p>
                            <p className="mt-2 text-sm">This is the commission per contract per round turn. E.g. if the fee is $2.50 to open and $2.50 to close, enter $5.00.</p>
                            <p className="text-sm italic">For example, if you enter $5.00 for ES, a trade of 2 contracts will have a total commission of $10.00.</p>
                            <div className="mt-4 space-y-2">
                                {Object.keys(missingCommissions).map(instrument => (
                                    <div key={instrument} className="flex items-center space-x-2">
                                        <label htmlFor={`commission-${instrument}`} className="min-w-[200px]">
                                            {instrument} - Commission per contract
                                        </label>
                                        <Input
                    ...
                    {processedTrades.length === 0 && (
                        <div className="flex-none bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded-r" role="alert">
                            <p className="font-bold">Duplicate Trades or Empty File</p>
                            <p>All trades in this file have already been imported, or the file contains no valid trades.</p>
                        </div>
                    )}
                    <div className="px-2">
