...
export class PDFExporter {
  ...
  addTradesTable(trades: TradeData[]) {
    ...
    const tableData = trades.map(trade => [
      ...
      trade.closePrice ? `$${formatNumber(trade.closePrice, 2)}` : 'Open',
  ...
  // Save the PDF
  save(filename: string = 'tradelytix-report.pdf') {
    this.addFooter()
...
export class ExcelExporter {
  ...
  constructor() {
    ...
    this.workbook.creator = 'Tradelytix'
  ...
  // Save the Excel file
  async save(filename: string = 'tradelytix-export.xlsx') {
